...
class Player(Entity):
    def __init__(self, position: tuple, groups, enemies_group, obstacle_sprites, create_attack, destroy_weapon, path = '../graphics/brawn_idle.png') -> None:
        self.max_stats: dict = {
            HEALTH: 100,
            ENERGY: 100,
            DAMAGE: 20,
    ...
    def input(self) -> None:
        ...
        if (keys[pygame.K_f] or pygame.mouse.get_pressed()[0]) and not self.attacking:
            if len(self.inventory.hotbar.content[self.inventory.hotbar.active_item_index]) >= 1 and issubclass(self.inventory.hotbar.content[self.inventory.hotbar.active_item_index][0].__class__, Weapon):
                self.attacking = True
        ...
        for skill_index in self.skills.keys():
            if self.skills[skill_index][SKILL_APPLIED]:
                ...
                if self.skills[skill_index][SKILL_ACTIVE]:
                    ...
                    if delta_time >= self.skills[skill_index][SKILL_COOLDOWN_DURATION] + self.skills[skill_index][SKILL_ACTIVE_DURATION]:
    ...
    def render_skills(self) -> None:
        ...
        for skill_index in self.skills:
            ...
            skill_rect = pygame.Rect(500 + 30 + (skill_index - DEC) * 50 + (skill_index - DEC) * 20, 800, 50, 50)
            
            display_surface.blit(skill_image, skill_rect)
            
            if self.skills[skill_index][SKILL_APPLIED]:
                if self.skills[skill_index][SKILL_ACTIVE]:
                    pygame.draw.rect(display_surface, SKILLS_ACTIVE_OUTLINE_COLOR, skill_rect, border_radius=SKILLS_BORDER_RADIUS, width=SKILLS_ACTIVE_OUTLINE_WIDTH)
                else:    
                    pygame.draw.rect(display_surface, SKILLS_COOLDOWN_OUTLINE_COLOR, skill_rect, border_radius=SKILLS_BORDER_RADIUS, width=SKILLS_COOLDOWN_OUTLINE_WIDTH)
            pygame.draw.rect(display_surface, SKILLS_OUTLINE_COLOR, skill_rect, border_radius=SKILLS_BORDER_RADIUS, width=SKILLS_OUTLINE_WIDTH)
    ...
    def auto_play_movement(self) -> None:
        ...
        if closest_enemy:
            ...
            if smallest_distance < ATTACK_DISTANCE:
