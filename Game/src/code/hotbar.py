...
class HotBar:
    ...
    def apply_active_item(self, player) -> None:
        if len(self.content[self.active_item_index]) and issubclass(self.content[self.active_item_index][0].__class__,
                                                                    Fruit):
    ...
    def insert(self, item: Item) -> bool:
        for i in range(self.capacity):
            if len(self.content[i]) and (
                    not issubclass(item.__class__, Weapon) and item.__class__ == self.content[i][0].__class__):
                self.content[i].append(item)
                return True

        for i in range(self.capacity):
            if not len(self.content[i]):
                self.content[i].append(item)
                return True
    ...
    def has_weapon(self) -> bool:
        if not self.content:
            return False
        for i in range(self.capacity):
            if len(self.content[i]) and issubclass(self.content[i][0].__class__, Weapon):
                return True
        return False
        ...
        for i in range(self.capacity):
            ...
            if self.active_item_index == i:
                pygame.draw.rect(surface, HOTBAR_ACTIVE_COLOR, ri, border_radius=HOTBAR_BORDER_RADIUS,
                                 width=HOTBAR_ACTIVE_WIDTH)
            pygame.draw.rect(surface, HOTBAR_OUTLINE_COLOR, ri, border_radius=HOTBAR_BORDER_RADIUS,
                             width=HOTBAR_OUTLINE_WIDTH)
