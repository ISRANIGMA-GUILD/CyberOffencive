...
class HotBar:
    ...
    def insert(self, item: Item) -> bool:
        for i in range(self.capacity):
            if len(self.content[i]) and (not issubclass(item.__class__, Weapon) and item.__class__ == self.content[i][0].__class__):
    ...
    def display(self) -> None:
        ...
        for i in range(self.capacity):
            ...
            if len(self.content[i]):
                ...
                rendered_amount_text_rect.topleft = (64 + i * 50 + i * 20, 800)
